      ...
      <NitroCardContentView overflow="hidden">
        ...
        <Grid columnCount={2} overflow="auto" className="nitro-group-members-list-grid">
          {membersData.result.map((member, index) => {
            return (
              <Flex key={index} gap={2} alignItems="center" overflow="hidden" className="member-list-item bg-muted rounded p-2">
                  ...
                  {member.rank !== GroupRank.REQUESTED && (
                    <Flex center>
                      <Base
                        ...
                        className={`icon icon-group-small-${member.rank === GroupRank.OWNER ? "owner" : member.rank === GroupRank.ADMIN ? "admin" : membersData.admin && member.rank === GroupRank.MEMBER ? "not-admin" : ""}`}
