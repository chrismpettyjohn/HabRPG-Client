import { IRoomSession, ItemConsumeComposer, RoomObjectVariable, RoomPreviewer, Vector3d } from "@nitrots/nitro-renderer";
...
export const InventoryFurnitureView: FC<InventoryFurnitureViewProps> = (props) => {
  ...
  const selectedItemId = selectedItem.items?.[0]?.id;
  const isConsumable = selectedItem?.items?.[0]?.consumable;
        ...
        {selectedItem && (
          <Column grow justifyContent="between" gap={2}>
            ...
            <Column gap={1}>
              ...
              {!!roomSession && isConsumable && (
                <Button variant="success" onClick={(event) => SendMessageComposer(new ItemConsumeComposer(selectedItemId))}>
                  Consume
                </Button>
              )}
