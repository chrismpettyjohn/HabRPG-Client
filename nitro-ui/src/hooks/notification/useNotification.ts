...
const useNotificationState = () => {
  ...
  useMessageEvent(UserDiedEvent, (event: UserDiedEvent) => {
    const parser = event.getParser();
    if (parser.data.attackerUserId === parser.data.victimUserId) {
      showSingleBubble(`${parser.data.victimUserName} commited suicide`, NotificationBubbleType.INFO);
      return;
    }
    showSingleBubble(`${parser.data.victimUserName} was killed by ${parser.data.attackerUserName}`, NotificationBubbleType.INFO);
  });
